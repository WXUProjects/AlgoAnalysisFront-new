...
| GET | `/user/blog/recommend` | 否 | 主站/发现推荐：全部 `visibility=public` 已通过审核文；query: `page`/`pageSize`/`orgId?`（私有域按组织过滤）/`excludeSolutions=1`（排除题解镜像，发现流去重） |
...
- **摘要统一不自动生成**（博客创建/更新与题解同步同一规则）：未填则存空字符串，不从正文截取；题解再更新只同步标题/正文，不覆盖作者手填摘要。
...
| GET | `/user/blog/theme/status` | 否 | query: `username?`；返回 `enabled`（遗留）、`themeId`（`chirpy` 默认 / `simple` / `mizuki`）、`subtitle`、`socialLinks` |
...
| GET | `/user/blog/admin/articles` | 站管 | 文章审核列表；query: `page`/`pageSize`/`keyword`/`status`/`visibility` |
| POST | `/user/blog/admin/moderate` | 站管 | body: `{ id, action: approve\|reject\|pending, note? }` |

**开通协议**：初次使用/发文/改外观前须签署；存量已有文章或主题配置用户启动时自动回填为已开通。

**审核**：`moderationStatus=approved|pending|rejected`；公开列表/广场仅 `approved`（空值兼容为已通过）。

**互动通知**（写主站 `notifications`）：博客点赞 `blog_article_like`、博客评论 `blog_comment` / `blog_comment_reply`；题解/评论点赞 `solution_like` / `comment_like`。payload 可含 `blogUsername`+`blogSlug` 跳转。邮件通知偏好默认关。

`GET /user/blog/by-username` 额外返回：`themeId`、`subtitle`、`socialLinks`（与 theme/status 一致）。

**主题说明**

| themeId | 名称 | 说明 |
|---------|------|------|
| `chirpy` | Chirpy | 默认；复刻 jekyll-theme-chirpy 布局 |
| `simple` | 简约 | 原 shadcn 顶栏壳 |
| `mizuki` | Mizuki | 可选；适配 [LyraVoid/Mizuki](https://github.com/LyraVoid/Mizuki)（MD3 气质，动画轻度–中度，无樱花/轮播等重特效） |
...
HTTP 手写路由。按前端 path **动态**生成 meta，**不**为每篇文章/题目生成静态文件。

| Method | Path | Auth | 说明 |
|--------|------|------|------|
| GET | `/user/seo/html` | 否 | query: `path`（前端路径，如 `/blog/u/slug`）；返回完整 HTML（title + description + OG/Twitter + 可引导 SPA）。**不**增加博客 `viewCount` |
| GET | `/user/seo/meta` | 否 | query: `path`；返回 JSON `{ title, description, image, url, type, siteName, author? }` |
| GET | `/user/seo/sitemap.xml` | 否 | 动态 sitemap：首页/广场/公开博文等 |

**图片回退**：文章封面 → 作者头像 → 站点 logo/favicon → `/favicon.png`。

**path 解析**：`/blog/:user/:slug`、`/blog/:user`、`/blog-plaza`、`/question-bank/detail/:id`、`/profile?id=`、`/p/:slug`（粘贴板，title 或语言+正文摘要）等；未知 path 用站点默认 meta。

nginx 建议：`/blog/*` 等公开页（所有人）反代到 `/api/user/seo/html?path=$request_uri`，见 `scripts/nginx-seo.conf.example`。
...
- `summary` 可选；空则列表不展示摘要，**不会**自动截取正文。
