...
**SpiderMonitorRes.data**（`GET /core/spider/monitor`；需 `site.config.read` / `site.spider.ops` / `site.problem.ops` 任一权限；运维页爬虫监控）
```json
{
  ...
  "platforms": [
    {
      ...
      "submitPaused": false,      "problemPaused": false,
      "paused": false
    }
  ]
}
...
- `submitPaused` / `problemPaused`：提交记录同步与题面获取的独立暂停状态
...
GET    /api/core/spider/luogu/resolve-user?query=...
