...
export function resolvePageTitle(pathname: string): string {
  ...
  if (path.includes('/solution/new')) return '写博客'
  if (path.includes('/solution/') && path.endsWith('/edit')) return '编辑博客'
  if (path.includes('/solution/')) return '博客'
  if (path === '/question-bank/add-problem') return '向题库加题'
  if (path.startsWith('/question-bank/detail/')) return '题目详情'
  if (path.includes('/add-problem')) return '向题单加题'
