...
4. **可说可不说的一律不说**：控件旁、表单下、禁用态旁，不要堆「解释实现/保证」类说明（如「创建后固定」「改标题不会改变链接」「仅首次填充」）。行为对就行，别教育用户；真正需要操作指引再写一句短的
...
- 可有可无的实现说明：「短链创建后固定，改标题不会改变链接」「自动绑定 · 本插件曾推送过」
...
| 软删除用户 | 移除用户 |
| 触发爬虫拉取 | 同步最新数据 |
| 短链创建后固定，改标题不会改变链接 | （删掉；编辑态禁用输入即可） |
...
- **搜索默认模糊**：列表/筛选用 `keyword`（或文档约定的 `q`），服务端 `ILIKE '%kw%'`；禁止只滤当前页；精确匹配须文档标明
...
core.contestCalendar: list, platforms, sub, subDelete
